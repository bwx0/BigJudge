...
"""
Randomly plays a valid combination.
Prefers to play combinations with more cards but the weakest one when playing first.
"""
            ...
            for clen in [5, 3, 2, 1]:
                ...
                for comb in combinations(sortedHand, clen):
                    ...
                    if not is_valid or trick_type == TrickType.PASS:
                        continue
                    valid_tricks.append((strength, comb))
                if len(valid_tricks) != 0:
                    action = valid_tricks[random.randint(0, len(valid_tricks) - 1)][1]
                    break
        else:
