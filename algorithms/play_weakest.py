...
"""
Always plays the weakest combination where possible.
Prefers to play combinations with less cards when playing first.
"""
    ...
    def getAction(self, state: MatchState):
        ...
        if sortedHand[0] == "3D":
            ...
            for clen in [1, 2, 3, 5]:
                for comb in combinations(sortedHand, clen):
                    ...
                    if not is_valid or trick_type == TrickType.PASS:
                        continue
                    valid_tricks.append((trick_type.value * 10 * 20 + strength, comb))
                if len(valid_tricks) != 0:
                    valid_tricks.sort(key=lambda x: x[0], reverse=False)
            ...
            for comb in combinations(sortedHand, card_size):
                ...
                if not is_valid or trick_type == TrickType.PASS:
                    continue
                if trick_type != t_trick_type:
                    continue
                if strength > t_strength and strength < wscore:
                    action = comb
